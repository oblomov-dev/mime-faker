CLASS z2ui5_file_cl_app_02 DEFINITION PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    DATA ms_file_popup     TYPE z2ui5_file_cl_db_api=>ty_s_file.

    DATA mt_file           TYPE STANDARD TABLE OF ty_s_file_out WITH EMPTY KEY.
    DATA ms_file_edit      TYPE z2ui5_file_cl_db_api=>ty_s_file.
    ...
    TRY.
        ...
        CASE client->get( )-event.
          ...
          WHEN 'UPLOAD'.
            ...
            DATA(lv_id_new) = z2ui5_file_cl_db_api=>create( VALUE #( data        = lv_data
                                                                     name        = mv_path
          ...
          WHEN 'BUTTON_METADATA_CONFIRM'.
            z2ui5_file_cl_db_api=>update_metadata( ms_file_popup ).
            COMMIT WORK AND WAIT.
            " TODO: check spelling: succesfully (typo) -> successfully (ABAP cleaner)
          ...
          WHEN 'EDIT'.
            ms_file_popup = z2ui5_file_cl_db_api=>read( id = ms_file-id ).
            CLEAR ms_file_popup-data.
            ui5_popup_metadata_display( ).

          WHEN 'EDITOR'.
            DATA(lo_Editor) = NEW z2ui5_file_cl_app_01( ).
            lo_editor->ms_file-id = mt_out[ selkz = abap_true ]-id.
            client->nav_app_call( lo_editor ).

          WHEN 'POPUP_BASE64'.
            DATA(lv_data3) = z2ui5_file_cl_db_api=>read( id = ms_file-id )-data.
...
*            )->link( text = 'Source_Code' href = view->hlp_get_source_code_url(  )
    ...
    DATA(tab) = page->table( headertext = 'Table'
